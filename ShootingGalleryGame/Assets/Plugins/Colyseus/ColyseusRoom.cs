...
namespace Colyseus
{
    ...
    public class ColyseusRoom<T> : IColyseusRoom
    {
        ...
        /// <param name="handler"></param>
        /// <typeparam name="MessageType">The type of object this message should respond with</typeparam>
        public void OnMessage<MessageType>(Action<MessageType> handler) where MessageType : ColyseusSchema, new()
        {
        ...
        protected async void ParseMessage(byte[] bytes)
        {
            ...
            {
                CSAIterator it = new CSAIterator {Offset = 1};
            ...
            else if (code == ColyseusProtocol.ROOM_DATA_SCHEMA)
            {
                ...
                ColyseusSchema message = (ColyseusSchema) Activator.CreateInstance(messageType);
