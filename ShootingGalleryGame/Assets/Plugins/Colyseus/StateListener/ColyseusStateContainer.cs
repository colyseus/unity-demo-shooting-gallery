...
namespace Colyseus
{
    using PatchListener = CSAListener<Action<CSADataChange>>;
    using FallbackPatchListener = CSAListener<Action<CSAPatchObject>>;
    ...
    public class ColyseusStateContainer
    {
        ...
        /// <returns>An array of <see cref="CSAPatchObject" /> representing the update</returns>
        public CSAPatchObject[] Set(IndexedDictionary<string, object> newData)
        {
            CSAPatchObject[] patches = ColyseusCompare.GetPatchList(state, newData);
        ...
        public FallbackPatchListener Listen(Action<CSAPatchObject> callback)
        {
        ...
        public PatchListener Listen(string segments, Action<CSADataChange> callback, bool immediate = false)
        {
        ...
        /// <summary>
        ///     Parse incoming raw rules and convert to <see cref="CSAListener{T}.Action{Colyseus.DataChange}}.rules" />
        /// </summary>
        ...
        private void CheckPatches(CSAPatchObject[] patches, List<PatchListener> _listeners = null)
        {
            if (_listeners == null)
            {
            ...
            for (int i = patches.Length - 1; i >= 0; i--)
            {
                ...
                for (int j = 0; j < _listeners.Count; j++)
                {
                    ...
                    {
                        CSADataChange dataChange = new CSADataChange
                        {
                            path = pathVariables,
        ...
        private Dictionary<string, string> GetPathVariables(CSAPatchObject patch, PatchListener listener)
        {
