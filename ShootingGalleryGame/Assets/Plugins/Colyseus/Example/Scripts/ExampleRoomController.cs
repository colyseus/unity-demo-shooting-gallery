...
public class ExampleRoomController
{
    ...
    public delegate void OnRoomStateChanged(ColyseusMapSchema<string> attributes);

    public delegate void OnRoundEnd(Winner winner);

    public delegate void OnScoreUpdate(ShootingGalleryScoreUpdateMessage update);

    public delegate void OnUserStateChanged(ColyseusMapSchema<string> changes);
    ...
    private void OnUserAdd(ExampleNetworkedUser user, string key)
    {
    ...
    /// <param name="key">The user key.</param>
    private void OnUserRemove(ExampleNetworkedUser user, string key)
    {
